...
if not isfolder('OBlock Sigma V2') then makefolder('OBlock Sigma V2') end
if not isfile('OBlock Sigma V2/configs') then
	makefolder('OBlock Sigma V2/configs')
end

if not isfile('OBlock Sigma V2/configs/globalConf.bin') then
	-- // By default global config is turned on
	writefile('OBlock Sigma V2/configs/globalConf.bin', 'true')
end

if not isfolder('OBlock Sigma V2/bypasses') then
	makefolder('OBlock Sigma V2/bypasses')
end

local globalConfFilePath = 'OBlock Sigma V2/configs/globalConf.bin'
...
local library = {
	...
	tabSize = 0,
	theme = {},
	foldername = isGlobalConfigOn and 'OBlock Sigma V2/configs/global' or string.format('OBlock Sigma V2/configs/%s', tostring(LocalPlayer.UserId)),
	fileext = '.json',
...
do -- // Load
	...
	do -- // Load Basics
		...
		local function toggleDetectionBypass(name, toggle)
			...
			writefile(string.format('OBlock Sigma V2/bypasses/%s.bin', name), tostring(toggle))
		end
		...
		local function getAllConfigs()
			...
			for _, v in next, listfiles('OBlock Sigma V2/configs') do
		...
		configSection:AddButton({
			...
			callback = function()
				...
				local configData = readfile(string.format('OBlock Sigma V2/configs/%s/%s', folderName, fullConfigName))
		...
		discordSection:AddButton({
			text = 'Join Discord',
			callback = function() return joinDiscord('W6ydE6MmNE') end
        })
		discordSection:AddButton({
			text = 'Copy Discord Invite',
			callback = function() return setclipboard('https://discord.gg/W6ydE6MmNE') end
		})
